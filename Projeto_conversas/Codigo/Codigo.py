...
class Funcoes():
    ...
    def widgets2(self):
        ...
        self.lb_codigo = Label(self.root2, text='DIGITA O CÓDIGO DA MENSAGEM', bg='#00aa88', fg='white',
                               font=('arial', 13, 'bold'))
        ...
        self.bt_clica = Button(self.root2, text='oK', bg='#00aa88', fg='white', font=('arial', 8, 'bold'),
                               command=self.pesquisar_msn)
        self.bt_clica.place(x=350, y=10)

        self.lb_clica = Label(self.root2, text='LISTAR TODAS AS MENSAGENS', bg='#00aa88', fg='white',
                              font=('arial', 13, 'bold'))
        self.lb_clica.place(x=10, y=50)

        self.bt_clica2 = Button(self.root2, text='oK', bg='#00aa88', fg='white', font=('arial', 8, 'bold'),
                                command=self.pesquisar_msn2)
...
class principal(Funcoes):
    ...
    def widgets(self):
        ...
        self.dialogo = Label(self.root, text='I n s i g h t s', bg=self.tela, fg='#0055d4',
                             font=('arial black', 45, 'bold'))
        self.dialogo.place(x=460, y=0)

        self.lb_codigo = Label(self.root, text='Código da Mensagem:', bg=self.tela, fg='#000080',
                               font=('arial', 15, 'bold'))
        self.lb_codigo.place(x=10, y=10)
        ...
        self.data = Button(self.root, text='DATA', bg=self.botao, fg='white', font=('arial', 15, 'bold'),
                           command=self.data_local)
        ...
        self.enviar = Button(self.root, text='ENVIAR', bg=self.botao, fg='white', font=('arial', 20, 'bold'),
                             command=self.inserir_dados)
        self.enviar.place(x=200, y=650)

        self.pesquisar = Button(self.root, text='PESQUISAR', bg=self.botao, fg='white', font=('arial', 20, 'bold'),
                                command=self.pesquisa)
        self.pesquisar.place(x=500, y=650)

        self.limpar = Button(self.root, text='LIMPAR', bg=self.botao, fg='white', font=('arial', 20, 'bold'),
    ...
    def data_local(self):
        ...
        self.entry_datalocal = Entry(self.root, textvariable=self.TextoLabel, bg=self.tela, fg='#0000aa',
                                     font=('arial', 18, 'bold'), relief=FLAT)
        self.entry_datalocal.place(x=110, y=155, width=120, height=35)

    def status(self):
        self.status_msn = ttk.Combobox(self.root, fg='#0000aa', font=('arial', 13, 'bold'),
                                       values=["Pendente", "Aberto", "Em Espera", "Respondido", "Ignorado"])
